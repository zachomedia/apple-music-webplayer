<template>
  <b-dropdown variant="link" size="sm" no-caret right boundary="window">
    ...
    <b-dropdown-item-button @click.stop="goToAlbum()" v-if="song.assets && song.assets[0].metadata">Go to album</b-dropdown-item-button>
    <b-dropdown-divider v-if="(isAuthorized && !isLibrary) || (song.assets && song.assets[0].metadata)" />
